...
const Modal = ({ visible, onClose, transaction, admin }) => {
	...
	const { mutateAsync: storeSignedData, isLoading } = useContractWrite(
		contract,
		"storeSignedData"
	);
	...
	const call = async (_hash, _sender, _imageName, _publicKey, _signature) => {
		try {
			const data = await storeSignedData({
				args: [_hash, _sender, _imageName, _publicKey, _signature],
			});
			Swal.fire({
				icon: "success",
