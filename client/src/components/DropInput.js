...
const DropImageInput = () => {
	...
	const { mutateAsync: storeImageData } = useContractWrite(
		contract,
		"storeImageData"
	);
	...
	const handleSubmit = async () => {
		...
		try {
			if (file) {
				// gọi api check AI
				const response = await axios.post("/v1/ggVision/verify", imageData, {
					headers: {
	...
	return (
		<>
			<div
				...
				className={`${
					blob ? "before-bg-file" : ""
				} relative p-6 cursor-pointer h-[400px] w-[570px] mx-auto mt-10 border-2 border-dashed  flex flex-col items-center justify-center text-base leading-[1.6] select-none`}>
				<input
					ref={inputFileRef}
			...
			{loading ? (
				...
				<button
					className="mt-12 m-auto flex bg-gradient-to-tr from-[#21D4FD] to-[#B721FF] px-12 py-4 text-xl rounded-lg"
					type="button"
